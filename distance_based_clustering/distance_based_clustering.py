...
SUPPORTED_DISTANCES = ['euclidean', 'cosine', 'cityblock']
SUPPORTED_SMALLSEG_MODES = ['merge', 'outlier']

def top_k(arr, k, axis = 0):
    """
    This function be used to zero out all but the top-k values in a numoy array

    :param arr: (np.ndarray)
    A numpy array which is to be thresholded to preserve the top-k values along an axis
    ...
    assert isinstance(k, int), "The <k> variable must be an integer"
    assert 0 < k < arr.shape[axis], f"The array has {arr.shape[axis]} elements in chosen axis, choose k between " \
                                    f"(0-{arr.shape[axis]})"

    'indices of top k values in axis'
...
class segment_set(object):
    ...
    def find(self, element_num):
        ...
        assert element_num in self.__segments.shape[0], f"<element_num> must be in range 0-{self.__segments.shape[0]-1}"

        y = int(element_num)
        while element_num != self.__segments[y, 0]:
    ...
    def seg_mst(self, segment_id):
        ...
        return self.__segments[segment_id, 2]
    ...
    def join(self, seg1, seg2, min_dist):
        ...
        if self.__segments[seg1, 0] < self.__segments[seg2, 0]:
            ...
            self.__segments[seg1, 1] += self.__segments[seg2, 1]
...
class distance_based_clustering(object):
    ...
    """
    def segment_graph(self, edges, seg_map=None, k=0.07):
        """
        ...
        assert isinstance(k, float) and (k> 0), "The variable <k> must be positive"
        ...
        if seg_map is None:
            seg_map = segment_set(num_elements=edges[:, 1].max())
        ...
        for ii in range(edges.shape[0]):
            ...
            if src_node != dest_node:
                ...
                if (seg_map.seg_mst(src_node) + seg_map.seg_mst(src_node) + edges[ii, 2])< k:
                    'If the MST is less than the specified limit MERGE the two nodes'
                    seg_map.join(src_node, dest_node, edges[ii, 2])
    ...
    def cleanup_smallsegments(self, edges, seg_map, mpix=15, cleanup_mode="outlier"):
        ...
        if cleanup_mode == "outlier":
            ...
            for ii in range(seg_map.num_segs()):
                if seg_map.size(ii) < mpix:
                    'Get the current member of segment'
                    current_nodes = np.where(segmentation == ii)
                    'Change label to -1 or outlier/unknown group'
                    segmentation[current_nodes] = -1

            return segmentation

    def segment_data(self, data, k=4, mpix=15, metric='cosine', cleanup_mode='outlier'):
        """
        ...
        :param k [0 < int < nSamples]
        The number of neighbors to add connections to in the graph created.

        :param mpix=15
        The number of pixels a cluster needs to have below which it is considered a small segment

        :param metric: ['str' in ['euclidean', 'cosine', 'cityblock']]
        ...
        assert 0 <= k, "The <num_nn> variable must be greater than 0"
        assert k <= data.shape[0], f"<num_nn> must be less than total number of samples. Currently, " \
                                        f"<num_nn>:{k} is greater than number of samples {data.shape[0]}"
        ...
        for ii in range(distances.shape[0]):
            'Find the k closest point for each'
            ind = [np.argpartition(distances[ii, :], k)[:k]]
            for item in ind:
                'If the edge does not already exist in the edges table add it'
                if np.asarray([min(ii, item), max(ii, item), distances[ii, item]]) not in edges:
                    edges += [np.asarray([min(ii, item), max(ii, item), distances[ii, item]])]

        'Sort the edges based on the edge weights'
        edges = edges[edges[:, -1].argsort()]

        'perform the segmentation for this graph using the distance'
        seg_map = self.segment_graph(edges, k=0.06)
